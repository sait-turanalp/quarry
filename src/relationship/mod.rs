...
/// Represents a node in a recursive call tree (downstream execution flow).
///
/// This is the inverse of impact analysis - instead of "who depends on me" (upstream),
/// this shows "what do I call" (downstream) with full recursive depth.
...
pub struct CallTreeNode {
    ...
    /// Child calls from this symbol (recursive structure)
    pub children: Vec<CallTreeNode>,

    /// Metadata about the call site (line, column, context)
    pub metadata: Option<RelationshipMetadata>,

    /// Whether this is an external/stdlib call (not in project index)
    pub is_external: bool,

    /// Whether this call creates a cycle (A → B → A)
    pub is_recursive: bool,

    /// Whether children were truncated due to depth/size limits
    pub truncated: bool,
...
/// Reason why a call tree node was truncated
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub enum TruncationReason {
    /// Maximum depth limit reached (max_depth parameter)
    MaxDepthReached,
    /// Recursive cycle detected (prevents infinite loops)
    CycleDetected,
    /// External library call (not in project, e.g. bcrypt, std::)
    ExternalCall,
}
