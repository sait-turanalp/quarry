...
pub trait LanguageParser: Send + Sync {
    ...
    /// Extract flow-oriented AST blocks for chunk-level retrieval.
    ///
    /// Default is empty so existing parsers remain backward compatible.
    fn find_flow_blocks(&mut self, _code: &str) -> Vec<FlowBlock> {
        Vec::new()
    }
}
