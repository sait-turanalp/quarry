...
impl TypeScriptParser {
    ...
    fn extract_symbols_from_node(
        ...
        match node.kind() {
            ...
            "interface_declaration" => {
                // Register ALL child nodes for audit
                self.register_node_recursively(node);
                self.process_interface_with_members(
                    node,
                    code,
                    file_id,
                    counter,
    ...
    /// Process an interface declaration and also index its property/method members as Field symbols.
    fn process_interface_with_members(
        &mut self,
        node: Node,
        code: &str,
        file_id: FileId,
        ...
        let interface_symbol = self.create_symbol(
            counter.next_id(),
        ...
        // Index property_signature and method_signature as Field symbols
        if let Some(body) = node.child_by_field_name("body") {
            for child in body.children(&mut body.walk()) {
                if child.kind() == "property_signature" || child.kind() == "method_signature" {
                    if let Some(member_name_node) = child.child_by_field_name("name") {
                        ...
                        let member_sig = code[child.byte_range()].trim().to_string();
                        ...
                        let mut member_symbol = Symbol::new(
                            counter.next_id(),
                            member_name,
                            kind,
                            file_id,
                        ...
                        member_symbol.signature = Some(member_sig.into());
                        // Members inherit parent visibility for type_fields,
                        // but should not appear as module-level exports
                        member_symbol.visibility = crate::Visibility::Private;
                        member_symbol.scope_context =
                            Some(crate::symbol::ScopeContext::ClassMember {
                                class_name: Some(interface_name.to_string().into()),
                            });
                        if !module_path.is_empty() {
                            member_symbol.module_path =
                                Some(format!("{module_path}.{interface_name}").into());
                        }
    ...
    fn extract_calls_recursive<'a>(
        ...
        // <Component /> and <Component>...</Component> are treated like function calls
        if node.kind() == "jsx_element" || node.kind() == "jsx_self_closing_element" {
            ...
            if let Some(comp_name) = component_name {
                // Only track React components (uppercase first char), skip HTML elements
                if comp_name.chars().next().is_some_and(|c| c.is_uppercase()) {
                    let context = function_context.or_else(|| {
                        // Ancestor inference for JSX context
                        let mut anc = node.parent();
                        while let Some(a) = anc {
                            match a.kind() {
                                "function_declaration" | "generator_function_declaration" => {
...
export function AnotherComponent() {
  return <Button>Another</Button>;
    ...
    #[test]
    fn test_typescript_find_flow_blocks_extracts_core_kinds() {
        let mut parser = TypeScriptParser::new().unwrap();
        let code = r#"
function run(input: string) {
  if (input) { doThing(); } else { other(); }
  for (const item of [1,2,3]) { handle(item); }
