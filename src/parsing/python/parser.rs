...
use crate::parsing::{
    FlowBlock, FlowKind, HandledNode, Language, LanguageParser, MethodCall, NodeTracker,
...
pub struct PythonParser {
    ...
    /// Track names listed in `__all__` for export visibility
    all_exports: Option<std::collections::HashSet<String>>,
}
...
impl PythonParser {
    ...
    pub fn parse(
        ...
        // Extract __all__ before processing symbols
        self.all_exports = Self::extract_dunder_all(root_node, code);
        ...
        // If __all__ is defined, mark only listed names as Public,
        // demote everything else at module scope to Module visibility
        if let Some(ref all_names) = self.all_exports {
            for symbol in &mut symbols {
                // Only affect module-scope symbols (not class members or locals)
                let is_module_scope = matches!(
                    symbol.scope_context,
                    Some(crate::symbol::ScopeContext::Module) | None
                );
                if !is_module_scope || symbol.kind == SymbolKind::Module {
                    continue;
                }
                if all_names.contains(symbol.name.as_ref()) {
                    symbol.visibility = Visibility::Public;
                } else if symbol.visibility == Visibility::Public {
                    symbol.visibility = Visibility::Module;
                }
            }
        }
    ...
    /// Extract `__all__` list from the module root.
    /// Returns Some(HashSet) if `__all__ = [...]` is found, None otherwise.
    fn extract_dunder_all(root: Node, code: &str) -> Option<std::collections::HashSet<String>> {
        for child in root.children(&mut root.walk()) {
            ...
            for stmt_child in child.children(&mut child.walk()) {
                ...
                let left = match stmt_child.child_by_field_name("left") {
                    Some(n) => n,
                    None => continue,
                };
                ...
                let right = match stmt_child.child_by_field_name("right") {
                    Some(n) => n,
                    None => continue,
                };
                if right.kind() != "list" {
                ...
                for elem in right.children(&mut right.walk()) {
                    if elem.kind() == "string" {
                        // Strip quotes from string content
                        let raw = &code[elem.byte_range()];
                        let unquoted = raw
                            .trim_start_matches(['"', '\''])
                            .trim_end_matches(['"', '\'']);
                        if !unquoted.is_empty() {
                            names.insert(unquoted.to_string());
                        }
                    }
                }
                if !names.is_empty() {
    ...
    fn extract_flow_blocks_recursive<'a>(
        ...
        match node.kind() {
            ...
            "match_statement" => blocks.push(FlowBlock::new(
                FlowKind::Switch,
                range,
                Some("match".to_string()),
                parent_symbol.clone(),
            )),
            "for_statement" | "while_statement" => blocks.push(FlowBlock::new(
                FlowKind::Loop,
                range,
                Some("loop".to_string()),
                parent_symbol.clone(),
            )),
            "raise_statement" => blocks.push(FlowBlock::new(
                FlowKind::ErrorPath,
                range,
            ...
            "call" => {
                if let Some(fn_node) = node.child_by_field_name("function") {
                    let callee = &code[fn_node.byte_range()];
                    if callee.contains('.') {
...
"#;
    ...
    #[test]
    fn test_python_find_flow_blocks_extracts_core_kinds() {
        let mut parser = PythonParser::new().unwrap();
        let code = r#"
def run(x):
    if x:
