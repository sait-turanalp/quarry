...
impl DocumentIndex {
    ...
    pub fn search(
        ...
        const BOOST_NAME_EXACT: f32 = 12.0;
        const BOOST_NAME_TEXT: f32 = 4.0;
        const BOOST_SIGNATURE: f32 = 2.5;
        const BOOST_DOC_COMMENT: f32 = 1.5;
        const BOOST_CONTEXT: f32 = 1.0;
        const BOOST_FUZZY_NGRAM: f32 = 0.35;
        const BOOST_FUZZY_WHOLE_WORD: f32 = 0.6;
        ...
        query_parser.set_field_boost(self.schema.name_text, BOOST_NAME_TEXT);
        query_parser.set_field_boost(self.schema.signature, BOOST_SIGNATURE);
        query_parser.set_field_boost(self.schema.doc_comment, BOOST_DOC_COMMENT);
        query_parser.set_field_boost(self.schema.context, BOOST_CONTEXT);
        ...
        let exact_name_query = TermQuery::new(
            Term::from_field_text(self.schema.name, query_str),
        ...
        all_clauses.push((
            ...
            Box::new(BooleanQuery::new(vec![
                ...
                (
                    Occur::Should,
                    Box::new(BoostQuery::new(
                        Box::new(fuzzy_whole_word_query),
                        BOOST_FUZZY_WHOLE_WORD,
                    )),
                ),
            ])),
        ));
...
mod tests {
    ...
    #[test]
    fn test_search_exact_name_boost_prioritizes_symbol_name_match() {
        let temp_dir = TempDir::new().unwrap();
        let settings = crate::config::Settings::default();
        let index = DocumentIndex::new(temp_dir.path(), &settings).unwrap();
        ...
        let exact_symbol_id = SymbolId::new(1001).unwrap();
        index
            .add_document(
                exact_symbol_id,
                "authenticate",
                ...
                20,
                0,
                Some("Complete authentication flow"),
                Some("function authenticate(token: string): Promise<Status>"),
                "auth",
                None,
                crate::Visibility::Public,
                Some(crate::ScopeContext::Module),
        ...
        let noisy_doc_symbol_id = SymbolId::new(1002).unwrap();
        index
            .add_document(
                noisy_doc_symbol_id,
                "helperUtility",
                ...
                0,
                Some(
                    "authenticate authenticate authenticate helper used for authentication middleware flows",
                ),
                Some("function helperUtility(): void"),
                "helpers",
                Some(
                    "This context repeatedly mentions authenticate but is not the actual symbol name",
                ),
                crate::Visibility::Public,
                Some(crate::ScopeContext::Module),
                Some("typescript"),
        ...
        let results = index.search("authenticate", 10, None, None, None).unwrap();
        assert!(
            !results.is_empty(),
            "expected at least one search result for authenticate"
        );
        assert_eq!(
            results[0].symbol_id, exact_symbol_id,
            "exact symbol name match should rank above noisy doc/context matches"
        );
    }
